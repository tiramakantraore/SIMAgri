    ...
    <div class="row">
        <div class="col-sm-12 col-md-12  ">
            <h2><g:message code="abondonnerRejets.Infos"/></h2>
        </div>
    </div>
    <div class="row">
        <div class="col-sm-4 col-md-4">
            <a  class="btn-flat  btn-primary abandoner">
                Abandonner les infos </a>
...
//            var quitter=$(".quitter");
            $(document).ready(function() {
           	jqGridPrice.jqGrid({
                    url:"${createLink(controller:'info', action:'populateRejetes')}",
                    datatype: 'json',
                ...
                caption: 'Abandon des informations (sélectionnez les infos à abandonner)',
                addCaption:' ',
                sortname: 'nomProduit',
                ...
                valider.click(function() {
                    ...
                    if (postData===""){
                    alert("Vous devez sélectionner au moins une info ");
                    }else {
                        selectedlist.val(postData);
                        valider.attr('disabled', 'disabled');
                        var jqxhr =$.ajax({
                            type: "POST",
                            url: "${createLink(controller:'info', action:'abandonner')}",
                            ...
                            $.jGrowl("L'abandon des infos a réussi", {
                            sticky: false,
                            header: 'Notification',
                            theme: 'iphone'
                            });
                            valider.removeAttr("disabled");
                            jqGridPrice.setGridParam({url: "${createLink(controller:'info', action:'populateRejetes')}"}).trigger("reloadGrid");
                        }).fail(function( jqXHR, textStatus, errorThrown ) {
                            $.jGrowl("L'abandon des infos a échoué, erreur rapportée "+errorThrown, {
                            sticky: true,
