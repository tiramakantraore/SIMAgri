...
var element,
    callbacks = {},
    resultsTitle, pivotTableId,
    resultsDivID,resultRowsId;
...
var methods = {
  setup   : function(options){
    ...
    else
    {
      ...
      if(options.pivotTableId){

          pivotTableId = options.pivotTableId;

      }
      else
      {
          pivotTableId = 'pivot-table';
      }
      if(options.resultRowsId){

          resultRowsId = options.resultRowsId;

      }
      else
      {
          resultRowsId = 'result-rows';
      }
  ...
  build_filter_field : function(fieldName, selectedValue) {
    ...
    var filterExists = $('#filter-list').find('select[data-field="' + field.name + '"]');
  ...
  build_select_filter_field : function(field, selectedValue){
    var snip  = '<select class="filter col-sm-3 col-md-3" '+(field.filterType==='multiselect'?'multiple':'')+' data-field="' + field.name + '">' +
                '<option></option>',
        orderedValues = [];
  ...
  build_regexp_filter_field : function(field, value){
    ...
    return '<input type="text" class="filter col-sm-3 col-md-3" data-field="' + field.name + '" value="' + value + '">';
  },
...
//      snip += '<b>Filters:</b> '    + filters + "<br/>"
  ...
  update_results : function(){
    ...
    snip += '<table id='+pivotTableId+' class="table table-condensed"><thead>';
    ...
    snip += '</thead></tr><tbody id='+resultRowsId+'></tbody></table>';
    result_table.append(snip);

    result_rows = $('#'+resultRowsId);
