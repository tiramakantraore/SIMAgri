  ...
  $.monthlyGraph = function(element, options) {
    ...
    var renderData = function() {
      ...
      var width = plugin.settings.width,
       ...
       // old   color = plugin.settings.graphColor;
        color="#FFC20F";
          r = Raphael(element, width, height);
          txt = {font: '12px Helvetica, Arial', fill: "#fff"}; // Label im Popup
          txtLabel = {font: '12px Helvetica, Arial', fill: "#648834"}; // Labelold color=#333
      ...
      label.hide();
      var frame = r.popup(100, 100, label, "right").attr({fill: "#000", stroke: "#4d883e", "stroke-width": 2, "fill-opacity": .7}).hide();//old stroke "#666"
      ...
      for (var i = 0, ii = graphData.labels.length; i < ii; i++) {
          ...
          var dot = r.circle(x, y, 4).attr({fill: "#ff860d", stroke: color, "stroke-width": 4}); // Datenpunkt old fill #f0f0f0
